...
export interface NotificationPayload {
  type: 'new_announcement' | 'updated_announcement' | 'featured_announcement' | 'urgent_announcement';
...
export class AnnouncementNotificationService {
   ...
   */
  async subscribeUser(userId: string, subscription: Partial<UserSubscription>): Promise<void> {
    try {
  ...
  async notifyUsers(payload: NotificationPayload): Promise<void> {
    try {
      ...
      this.logger.debug(`Sending ${payload.type} notification to ${targetUsers.length} users`);
      ...
      this.logger.log(`Successfully sent notifications for announcement ${payload.announcement.id}`);
    } catch (error) {
   ...
   */
  async notifyUrgentAnnouncement(announcement: EventAnnouncement): Promise<void> {
    try {
   ...
   */
  async notifyFeaturedAnnouncement(announcement: EventAnnouncement): Promise<void> {
    try {
      ...
      await this.notifyUsers(payload);
      this.logger.log(`Sent featured notification for announcement ${announcement.id}`);
    } catch (error) {
      this.logger.error(`Failed to send featured notification: ${error.message}`);
      throw error;
    }
  }
  ...
  registerSocketConnection(userId: string, socket: any): void {
    try {
      ...
      this.logger.error(`Failed to register socket for user ${userId}: ${error.message}`);
    }
  }
  ...
  removeSocketConnection(userId: string): void {
    try {
      ...
      this.logger.error(`Failed to remove socket for user ${userId}: ${error.message}`);
    }
  }
  ...
  getNotificationStats(): Record<string, any> {
    ...
    const subscriptionsWithEmail = Array.from(this.userSubscriptions.values())
      .filter(s => s.preferences.email).length;
    const subscriptionsWithPush = Array.from(this.userSubscriptions.values())
    ...
    return {
      ...
      connectionRate: totalSubscribers > 0 ? (activeConnections / totalSubscribers) * 100 : 0
    };
  }
  ...
  private getTargetUsers(payload: NotificationPayload): UserSubscription[] {
    ...
    return Array.from(this.userSubscriptions.values()).filter(subscription => {
      ...
      if (socket && socket.connected) {
        try {
          socket.emit('announcement_notification', {
            type: payload.type,
            announcement: this.formatAnnouncementForNotification(payload.announcement),
            priority: payload.priority,
            timestamp: new Date()
          });
        } catch (error) {
          this.logger.warn(`Failed to send real-time notification to user ${user.userId}: ${error.message}`);
        }
      }
    }

    this.logger.debug(`Sent real-time notifications to ${realTimeUsers.length} users`);
  }
  ...
  private async sendEmailNotifications(
    ...
    this.logger.debug(`Would send email notifications to ${emailUsers.length} users`);
  ...
  private async sendPushNotifications(
    ...
    this.logger.debug(`Would send push notifications to ${pushUsers.length} users`);
  ...
  private formatAnnouncementForNotification(announcement: EventAnnouncement): Record<string, any> {
    return {
      id: announcement.id,
      title: announcement.title,
      summary: announcement.summary || announcement.content.substring(0, 200) + '...',
